    ...
    if message.guild then --if the message was sent in a guild
        ...
        if #mention == 1 and message.guild:getMember(mention[1]):hasPermission(0x00000008) then --true if 1 and ONLY 1 user was mentioned in 'message'
            ...
            if line1 == '1' then --true if the guild already has a permanent DM
                ...
                return message:reply(currentDm .. " is the permanent DM for this server. If you feel this is in error, reroll a bard and cry for attention.")
            elseif line1 == '0' or line1 == nil then --true if the guild does NOT already have a permanent DM
                ...
                guildDmRecord:close()
                return message:reply(mention[1].name .. " successfully set as this server's current DM.")
            else
                ...
                guildDmRecord:write('0\n' .. mention[1].id)
                guildDmRecord:close()
                return message:reply("Successfully overwrote corrupted guild file. " .. mention[1].name .. " set as this server's current DM.")
            end
        elseif #mention == 0 then --true if no user was mentioned
            ...
            elseif line1 == '0' then --true if the guild has a temporary DM
                print('temporary DM')
                local currentDm = message.guild:getMember(line2).name
                return message:reply(currentDm .. ' is currently the DM. If you were trying to change that, RTFM.')
            else --true if line1 could not be interpreted, and therefore the file is either corrupt or empty
                print('no DM')
                return message:reply("Ummm... you kinda have to... like... tell me who you want to the DM to be...")
            end
        elseif #mention > 1 then --true if multiple users were mentioned
            print('multiple mentions')
            return message:reply('Whoa, now. Multiple dungeon masters is NOT as kinky as you might think.')
        else --length of mention fucked up
            return message:reply('Error dm-1: \"How the fuck is there a negative number of mentioned users?!?!?!\" ' .. client.owner.mentionString .. " Please help, Daddy. O_O")
        end
...
local function pdm(client, logger, message, text)
    if message.guild then --if the message was sent in a guild
        ...
        if message.author.id == message.guild.ownerId then --message author is guild owner
            ...
            guildDmRecord:close()
            return message:reply('***__ALL HAIL OUR GOD AND DAMNATION, ' .. message.author.name:upper() .. "!!!__***")
...
local function spam(client, logger, message, text)
    ...
    if message.guild then
        if message.guild:getMember(message.author):hasPermission(0x00000008) then
            ...
            return message:reply("You don't have the authority to have me spam for you.")
        end
    else
        if text ~='' then
            return loop(message.channel, text)
